      ...
      for (const archive of archivesA) {
        for (const account of archive.accounts) {
          this.allAccounts.set(account.code, {
            ...
            // إضافة حالة افتراضية نشطة للحسابات الأرشيفية
...
• التالي: ${this.getNextCode()}`;
  ...
  // تعديل دالة عرض الأرشيف لعرض جميع الحسابات
  formatArchiveDisplay(archiveData) {
    ...
    // عرض جميع الحسابات دون اقتصاص
    archiveData.accounts.forEach(account => {
      // عرض فقط الحسابات النشطة (لا نعرض أيقونة محظور للحسابات الأرشيفية)
