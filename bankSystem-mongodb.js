    ...
    try {
      ...
      if (account.source === 'archive') {
        ...
        if (!dbAccount) {
          ...
          const [success, response] = await this.createAccountWithPassword(userId, account.username, newPassword, code);
          if (!success) {
            return response;
          }
...
🔒 النظام يدعم جميع الحسابات من الأرشيفات A و B`;
  ...
  async handleLogin(userId, command) {
    ...
    try {
      ...
      // إذا كان الحساب من الأرشيف
      if (account.source === 'archive') {
        // البحث أولاً في قاعدة البيانات لمعرفة إذا كان الحساب مفعل
        const dbAccount = await this.db.getAccountByCode(code);
        
        if (!dbAccount) {
          // الحساب من الأرشيف ولم يتم تفعيله بعد
          return `❌ الحساب ${code} موجود في الأرشيف لكن لم يتم تفعيله بعد.\n\n📞 تواصل مع المسؤول لتفعيل الحساب.`;
        } else {
          // الحساب مفعل، نتحقق من كلمة السر
  ...
  async handleCreateWithPassword(userId, command) {
    ...
    const [success, response] = await this.createAccountWithPassword(userId, username, password);
...
🔒 احفظ كودك وكلمة السر`;
  ...
  // عرض إجمالي الغولد في الأرشيف المحدد
  async handleArchiveTotal(userId, command) {
    if (!this.isSuperAdmin(userId)) {
      return `❌ هذا الأمر للمدير الأساسي فقط`;
    }
    
    const match = command.match(/مجموع\s+([AB])\s*(\d+)/i) || 
                  command.match(/مجموع\s+([AB])(\d+)/i);
    ...
    try {
      ...
      const totalGold = archive.accounts.reduce((sum, acc) => sum + acc.balance, 0);
      const accountCount = archive.accounts.length;
      const averageBalance = accountCount > 0 ? Math.round(totalGold / accountCount) : 0;
      
      return `💰 إجمالي الغولد في الأرشيف ${archive.name}:

• إجمالي الغولد: ${totalGold.toLocaleString()} ${config.currency}
• عدد الحسابات: ${accountCount.toLocaleString()}
• متوسط الرصيد: ${averageBalance} ${config.currency}
• النطاق: ${archive.start} - ${archive.end}`;
    } catch (error) {
      console.error('❌ خطأ في حساب مجموع الأرشيف:', error);
      return `❌ حدث خطأ في حساب مجموع الأرشيف`;
    }
  }

  // عرض أعلى 10 حسابات في الأرشيف المحدد
  async handleArchiveTop(userId, command) {
    if (!this.isSuperAdmin(userId)) {
      return `❌ هذا الأمر للمدير الأساسي فقط`;
    }
    
    const match = command.match(/توب\s+([AB])\s*(\d+)/i) || 
                  command.match(/توب\s+([AB])(\d+)/i);
    ...
    try {
      ...
      const sortedAccounts = archive.accounts
        .filter(acc => acc.balance > 0)
        .sort((a, b) => b.balance - a.balance)
        .slice(0, 10);
      
      if (sortedAccounts.length === 0) {
        return `📊 لا توجد حسابات نشطة في الأرشيف ${archive.name}`;
      }
      
      let topText = `🏆 أعلى 10 حسابات في الأرشيف ${archive.name}:\n\n`;
      ...
      console.error('❌ خطأ في عرض توب الأرشيف:', error);
      return `❌ حدث خطأ في عرض توب الأرشيف`;
    }
  }

  // عرض إجمالي الغولد في النظام (للمدير الأساسي فقط)
...
• الحسابات النشطة: ${allAccounts.filter(acc => acc.balance > 0 && acc.status !== 'banned').length} حساب
• الحسابات المحظورة: ${bannedAccounts.length} حساب`;
    } catch (error) {
  ...
  async handleBannedUsers(userId) {
    try {
      const allAccounts = await this.getAllAccounts();
      // عرض فقط الحسابات المحظورة فعلياً في قاعدة البيانات
      const bannedAccounts = allAccounts.filter(acc => acc.status === 'banned' && acc.source === 'database');
...
`;
    ...
    try {
      const allAccounts = await this.getAllAccounts();
      const activeAccounts = allAccounts.filter(acc => acc.balance > 0 && acc.status !== 'banned').length;
      const totalGold = allAccounts.reduce((sum, acc) => sum + acc.balance, 0);
...
• متوسط الرصيد: ${averageBalance} ${config.currency}`;
  ...
  async handleArchive(userId, command) {
    ...
    try {
      ...
      // عرض معلومات الأرشيف فقط بدون عرض الحسابات
      return `📁 ${archive.name}
📍 من ${archive.start} إلى ${archive.end}
📊 يحتوي على ${archive.accounts.length} حساب`;
    } catch (error) {
...
`;
      ...
      if (this.hasPermission(userId, 'انشاء')) {
        ...
        helpText += `• انشاء [الاسم] [كلمة السر] - إنشاء حساب بكلمة سر\n`;
      }
      ...
      if (this.hasPermission(userId, 'ارشيف')) {
        helpText += `• ارشيف [A/B][رقم] - عرض معلومات الأرشيف\n`;
      }
...
• مجموع - إجمالي الغولد في النظام
• مجموع [A/B][رقم] - إجمالي الغولد في الأرشيف المحدد
• توب - أعلى 10 حسابات في النظام
• توب [A/B][رقم] - أعلى 10 حسابات في الأرشيف المحدد
...
• التالي: ${this.getNextCode()}`;
  ...
  // إنشاء حساب بدون كلمة سر (النظام القديم)
  ...
  // إنشاء حساب بكلمة سر (النظام الجديد)
  ...
  async transferMoney(fromUser, toCode, amount) {
    ...
    try {
      // إذا كان الحساب المستلم من الأرشيف، نحتاج إلى إنشاؤه في قاعدة البيانات أولاً
      if (toAccount.source === 'archive') {
        const dbToAccount = await this.db.getAccountByCode(toCode);
        if (!dbToAccount) {
          return [false, "❌ الحساب المستلم موجود في الأرشيف لكن لم يتم تفعيله بعد.\n📞 تواصل مع المسؤول لتفعيل الحساب."];
        }
      }
  ...
  async banAccount(adminId, code) {
    ...
    try {
      ...
      if (account.source === 'archive') {
        const dbAccount = await this.db.getAccountByCode(code);
        if (!dbAccount) {
          return [false, "❌ الحساب موجود في الأرشيف لكن لم يتم تفعيله بعد.\nاستخدم أمر 'ربط' لتفعيل الحساب أولاً."];
        }
      }
  ...
  async linkAccount(code, targetUserId, password) {
    ...
    try {
      // إذا كان الحساب من الأرشيف، ننشئه في قاعدة البيانات
      if (account.source === 'archive') {
        const [success, response] = await this.createAccountWithPassword(targetUserId, account.username, password, code);
        if (!success) {
