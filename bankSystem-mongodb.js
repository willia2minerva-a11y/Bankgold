...
class BankSystem {
  constructor() {
    ...
    console.log(`📊 التالي: ${this.getNextCode()}`);
  ...
  // تفعيل حساب من الأرشيف تلقائياً عند الحاجة
  async activateArchiveAccount(account, userId = null, password = null) {
    try {
      // إذا كان الحساب من الأرشيف ولم يكن مفعلاً
      if (account.source === 'archive') {
        ...
        if (!dbAccount) {
          ...
          await this.db.createAccount(
            userId || account.user_id || config.adminUserId,
            account.code,
            account.username,
            passwordHash,
            account.balance
  ...
  async handleChangePassword(userId, command) {
    ...
    try {
      ...
      // تفعيل الحساب من الأرشيف إذا لزم الأمر
      if (account.source === 'archive') {
...
`;
      ...
      if (this.hasPermission(userId, 'ارشيف')) {
        helpText += `• ارشيف [A/B][رقم] - عرض الأرشيف كاملاً\n`;
      }
...
• التالي: ${this.getNextCode()}`;
  ...
  async transferMoney(fromUser, toCode, amount) {
    ...
    try {
      // تفعيل الحساب المستلم من الأرشيف إذا لزم الأمر
  ...
  async linkAccount(code, targetUserId, password) {
    ...
    try {
      // تفعيل الحساب من الأرشيف مع المستخدم الجديد
